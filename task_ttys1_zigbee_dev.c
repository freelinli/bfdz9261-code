...
#define LESS_THAN_MIN_OPEN 30
#define LESS_THAN_MIN_CLOSE 40
								...
								if(JsonData4 != NULL)
								{
									if((cJSON_GetObjectItem(JsonData4, "acon")!= NULL)&&(cJSON_GetObjectItem(JsonData4, "acon")->valueint == 2))
									{
												...
												|| ((now_time > st3_time)&&(now_time < et3_time)))
										{
											...
											if((json_trans1 = GetJsonObject(SHARE_MEM_FIEL)) != NULL)
											{
												LOG_DEBUG(TAG,"json_trans1 ok\r\n");
												if(((json_trans2 = cJSON_GetObjectItem(json_trans1, "transformer")) != NULL) && (cJSON_GetArraySize(json_trans2) != 0))
												{
													LOG_DEBUG(TAG,"json_trans2 ok\r\n");
													for(k = 0; k < cJSON_GetArraySize(json_trans2); k ++)
													{

														LOG_DEBUG(TAG," id = %d a6 =%d\r\n", cJSON_GetObjectItem(cJSON_GetArrayItem(json_trans2, k), "id")->valueint, 	cJSON_GetObjectItem(JsonData4, "a6")->valueint);
														if( cJSON_GetObjectItem(cJSON_GetArrayItem(json_trans2, k), "id")->valueint ==
																cJSON_GetObjectItem(JsonData4, "a6")->valueint)
														{
															json_trans3 = cJSON_GetObjectItem(cJSON_GetArrayItem(json_trans2, k), "data");

															LOG_DEBUG(TAG,"json_trans3 ok\r\n");
															if(json_trans3 != NULL)
															{
																if((cJSON_GetObjectItem(JsonData4, "hgq")!= NULL))
																{

																	LOG_DEBUG(TAG, "hgqsize = %d\r\n", cJSON_GetArraySize(cJSON_GetObjectItem(JsonData4, "hgq")));
																	if(cJSON_GetArraySize(cJSON_GetObjectItem(JsonData4, "hgq")) == 1) // one or two relay
																	{
																		temp = cJSON_GetArrayItem(cJSON_GetObjectItem(JsonData4, "hgq"), 0)->valueint;

																		LOG_DEBUG(TAG, "hgq = %d\r\n", temp);
																		if((cJSON_GetArrayItem(json_trans3, (temp / 8))->valueint) & ( 0x1 << (temp % 8)))
																		{
																			//	flag_on_off = 1;
																	...
																	}else if(cJSON_GetArraySize(cJSON_GetObjectItem(JsonData4, "hgq")) == 4) // three relay
																	{
																		...
																		LOG_DEBUG(TAG, "hgq VALUE =%02x\r\n", temp);
																		LOG_DEBUG(TAG, "buf_control_dev_by_time[cJSON_GetObjectItem(JsonData4, \"a6\")->valueint][temp / 8]) =%02x\r\n", 
																				buf_control_dev_by_time[cJSON_GetObjectItem(JsonData4, "a6")->valueint][temp / 8]);
																		LOG_DEBUG(TAG, "0x01 <<(temp % 8) =%02x\r\n", (0x01 <<(temp % 8)));

																		if((buf_control_dev_by_time[cJSON_GetObjectItem(JsonData4, "a6")->valueint][temp / 8]) & (0x01 <<(temp % 8)))
																		{

																			temp = 	cJSON_GetArrayItem(cJSON_GetObjectItem(JsonData4, "hgq"), 2)->valueint;
																			...
																			buf_control_dev_by_time[cJSON_GetObjectItem(JsonData4, "a6")->valueint][(temp / 8)] &= (~(0x01 <<(temp % 8)));
																			LOG_DEBUG(TAG, "buf_control_dev_by_time[cJSON_GetObjectItem(JsonData4, \"a6\")->valueint][temp / 8]) =%02x\r\n", 
																					buf_control_dev_by_time[cJSON_GetObjectItem(JsonData4, "a6")->valueint][(temp / 8)]);
																		}else
																		{
...
send_control_again:
																			...
																			LOG_DEBUG(TAG, "control_dev_by_time data send cmd 3 delay -->\t");
																			for(x = 0; x < 10; x ++)
																				printf("%02x ", buf_all_data[x]);
																			printf("\r\n");
																			...
																			buf_all_data[4] = 0x02;
																			buf_all_data[5] = 0x00;
																			temp = CRC16_Change(buf_all_data, 6);
																			buf_all_data[6] = (temp &0x0000ff00 )>>8;
																			buf_all_data[7] = (temp &0x000000ff);
																			buf_all_data[8] = 0xbf;

																			LOG_DEBUG(TAG, "control_dev_by_time data get transformer -->\t");
																			for(x = 0; x < 10; x ++)
																				printf("%02x ", buf_all_data[x]);
																			printf("\r\n");
																			...
																			{		
																				...
																				if (nread > 0){
																					...
																					LOG_DEBUG(TAG,"read transformer DATALen=%d\n",nread); 
																					LOG_DEBUG(TAG,"read transformer data:\r\n"); 

																					for(m = 0; m < nread; m++)
																						printf("%02x ", Buf_Recv[m]);
																					printf("\r\n");
																				...
																				}else
																				{
																					...
																					if(flag_three_times ++ > 2)
																						break;
																					if(time_elapsed > 5000) // 5s
																						goto send_control_again;	
